...
def main(args):
    ...
    if (args.mode=='TRAIN'):
        ...
        for im in valid_lr_imgs:
            ...
            im = imresize(im, size=[128, 128], interp='bicubic', mode=None)
            tl.vis.save_image(im, args.lr_bicu_dir + '/' + valid_lr_img_list[i])
            sr_image,sr_ti = fsrgan.LR_to_HR(im)
        ...
        data_frame.to_csv(args.statistics_dir + '/' + 'test_resultsz_fsr.csv')
    else:
        ...
        for i in range(nrof_classes):
            ...
            for im in valid_lr_imgs:
                im = imresize(im, size=[32, 32], interp='bicubic', mode=None)
                im = imresize(im, size=[128, 128], interp='bicubic', mode=None)
                tl.vis.save_image(im, args.lr_bicu_dir + '/' + class_name+ '/'+valid_lr_img_list[k])
